...
def draw_cd_diagram(
    df_perf=None, alpha=0.05, title=None, labels=False, colname="accuracy"
):
    """
    ...
    p_values, average_ranks, _ = wilcoxon_holm(
        df_perf=df_perf, alpha=alpha, colname=colname
    )
...
def wilcoxon_holm(alpha=0.05, df_perf=None, colname=None):
    ...
    for i in range(m - 1):
        ...
        perf_1 = np.array(
            df_perf.loc[df_perf["classifier_name"] == classifier_1][colname],
            dtype=np.float64,
        )
        for j in range(i + 1, m):
            ...
            # get the performance of classifier one
            perf_2 = np.array(
                df_perf.loc[df_perf["classifier_name"] == classifier_2][colname],
                dtype=np.float64,
            )
